...
public class AlumnoServicio 
{
    ...
    /**
     * Este método permite matricular a un alumno en un módulo. Deben existir ambos y se identifican por su PK
     * @param idAlumno
     * @param idModulo
     */
    public void matricularAlumnoEnModulo(Long idAlumno, Long idModulo) 
    {
        EntityManager em = JPAUtil.getEntityManager();
        EntityTransaction trans = em.getTransaction();
        try 
        {
            ...
            if (alumno == null) 
            {
                LOG.warn("El alumno al que matricular no existe.");
                trans.rollback();
                return;
            }

            ModuloDAO moduloDAO = new ModuloDAO(em);
            // 2º Buscamos modulo existente
            Modulo modulo = moduloDAO.buscarPorId(idModulo);
            if (modulo == null) 
            {
                LOG.warn("El modulo sobre el que matricular al alumno no existe.");
                trans.rollback();
                return;
            }

            // Nosotros debemos relacionar las entidades alumno y Modulo:
            alumno.aniadirModulo(modulo);
            modulo.aniadirAlumno(alumno);
            
            trans.commit();

            LOG.info("Matricula realizada del alumno["+idAlumno+"] en Modulo["+ idModulo+"]");
        } 
        catch (RuntimeException e) 
        {
            LOG.error("Error durante la transacción: "+ e.getMessage());
